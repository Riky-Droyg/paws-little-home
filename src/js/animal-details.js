...
<div class="modal__animal-description">
  <p class="modal__specie" id="modal-species">${animal.species}</p>

  <h3 class="modal__name" id="modal-title">${animal.name}</h3>

  <div class="modal__info" aria-label="Коротка інформація">
    <p class="modal__age">
      <span class="sr-only">Вік: </span>${animal.age}
    </p>
    <p class="modal__gender">
      <span class="sr-only">Стать: </span>${animal.gender}
    </p>
  </div>

  <h4 class="modal__heading" id="modal-desc-title">Опис:</h4>
  <p class="modal__details" id="modal-desc">${animal.description}</p>
  ...
  <button
    ...
    class="modal__btn"
    id="modal-btn"
    aria-label="Взяти ${animal.name} додому"
  >
    Взяти додому
  </button>
</div>
